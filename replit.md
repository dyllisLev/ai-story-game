...
- `stories` - Story metadata (title, description, genre, author, timestamps)
- `messages` - Chat history with story ID foreign key, role (user/assistant), content, and optional character attribution
...
1. User creates or selects a story through the React frontend
2. Story metadata stored in SQLite via Drizzle ORM
3. During gameplay, messages are sent to the selected AI model's API
4. AI responses are saved to the messages table with story association
5. TanStack Query manages caching and synchronization between UI and API
